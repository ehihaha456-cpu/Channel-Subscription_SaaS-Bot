...
async def handle(self, update, context, q, owner, staff, a, role):
    ...
    if a == 'a_channel_resend_yes':
        ...
        for sub in subscriptions:
            ...
            for ch in channels:
                try:
                    invite = await context.bot.create_chat_invite_link(chat_id=ch['chat_id'], member_limit=1)
                    await save_invite(owner, user_id, ch['chat_id'], invite.invite_link)
                    link_lines.append(f"📢 {ch.get('title', 'Premium Channel')}\n{invite.invite_link}")
                except Exception as exc:
                    invite_failed += 1
                    logger.warning('Invite create failed owner=%s chat=%s user=%s: %s', owner, ch.get('chat_id'), user_id, exc)
            if not link_lines:
                failed += 1
                continue
            try:
                await context.bot.send_message(chat_id=user_id, text=f'📢 Channel/Group Invite Links Updated\n\nYour subscription is still active.\n\n⏱ Remaining: {days}d {hours}h {minutes}m\n\nJoin using the fresh invite link(s):\n\n' + '\n\n'.join(link_lines), disable_web_page_preview=True)
                sent += 1
            except Exception as exc:
                failed += 1
    ...
    if a == 'a_retry_failed':
        ...
        for item in failed_docs:
            ...
            try:
                links = []
                for ch in channels:
                    invite = await context.bot.create_chat_invite_link(ch['chat_id'], member_limit=1)
